...
const firedEvents: Set<string> = new Set();
 ...
 */
function onEvent(eventName: string, handler: Handler) {
 ...
 */
function onEvents(events: EventName | EventName[], handler: Handler) {
  ...
  eventList.forEach(eventName => {
    onEvent(eventName, () => {
      received.add(eventName);
      if (received.size === total) handler();
    });
  });
}
...
if (isBrowser() && !window.eventLayer._intercepted) {
  ...
  window.eventLayer.push = function (eventName: string) {
    // Adiciona ao array
