 ...
 * ----------------------------------------------------------------- */
export async function goPageCheckout() {
    if (!isBrowser())
        return;
    ...
    try {
        ...
        if (!res.ok)
            throw new Error(`Falha ao processar checkout: ${res.statusText}`);
 ...
 * ----------------------------------------------------------------- */
export async function goPageAccept() {
    if (!isBrowser())
        return;
    const visitor_id = getStorageItem('visitor_id');
    if (!visitor_id)
    ...
    try {
        ...
        if (!res.ok)
            throw new Error('Accept funnel failed');
 ...
 * ----------------------------------------------------------------- */
export async function goPageReject() {
    if (!isBrowser())
        return;
    const visitor_id = getStorageItem('visitor_id');
    if (!visitor_id)
    ...
    try {
        ...
        if (!res.ok)
            throw new Error('Reject funnel failed');
        const response = await res.json();
        // Se sucesso, redireciona para a URL de sucesso
        if (response.success) {
            window.location.href = response.redirect;
